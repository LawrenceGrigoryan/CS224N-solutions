...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        doc = self.data[idx]
        trunc_idx = random.randint(4, int(self.block_size*7/8))
        doc_trunc = doc[:trunc_idx]

        avg_len = int(len(doc_trunc) / 4)
        mask_len = avg_len + random.randint(-avg_len, avg_len)
        prefix_idx = random.randint(0, int((len(doc_trunc) - mask_len) / 2))

        prefix = doc_trunc[:prefix_idx]
        masked_content = doc_trunc[prefix_idx:prefix_idx+mask_len]
        suffix = doc_trunc[prefix_idx+mask_len:]

        corrupted_string = prefix + self.MASK_CHAR + \
            suffix + self.MASK_CHAR + \
            masked_content
        corrupted_string = corrupted_string + self.PAD_CHAR*(self.block_size - len(corrupted_string))

        x, y = corrupted_string[:-1], corrupted_string[1:]
        x_idx = [self.stoi[ch] for ch in x]
        y_idx = [self.stoi[ch] for ch in y]
